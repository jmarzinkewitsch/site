...
| **Schlagzeile** | **eine** Headline + 1–2 Sätze (eine Tafel je Story; Quelle als Label) | RSS-Feeds, verschränkt: ZEIT + NDR Hamburg (in vault-api) | ~14 s | überspringen |
...
2. **News:** **mehrere RSS-Feeds** direkt in vault-api, verschränkt — Default
   **ZEIT** (`newsfeed.zeit.de/all`, überregional) + **NDR Hamburg** (lokal);
   je Feed Top ~3, Quelle als Gleis-Label, eine Tafel pro Story. Konfigurierbar.
   Die zwei `input_text.hamburg_news_*`-Entitäten fallen raus (mismatchte
   100-Zeichen-Einzeiler).
