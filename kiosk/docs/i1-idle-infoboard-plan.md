...
- `headlines`: **schon gebaut** — einfach `await services.news.fetch_headlines(http, config.kiosk_news.feeds, per_feed=config.kiosk_news.per_feed)` aufrufen → `list[NewsHeadline]` (`{title, summary, source, link}`), schon verschränkt (ZEIT · NDR · ZEIT · NDR), RSS 2.0 + RDF, HTML gestrippt. `source` → Gleis-Label. **Nur in `headlines[]` mappen.** (Optional 60 s–5 min cachen.)
