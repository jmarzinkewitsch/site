...
- **News:** **mehrere RSS-Feeds** serverseitig in vault-api, verschränkt — Default **ZEIT** (überregional) + **NDR Hamburg** (lokal); Quelle als Gleis-Label. Konfigurierbar (`kiosk_news.feeds`). **Nicht** die zwei `input_text`-Entitäten.
...
GET /kiosk/idle/overview        (require_bearer_or_kiosk)
{
  ...
  "headlines": [ { "title": string, "summary": string, "source": string }, … ],  // verschränkt aus mehreren Feeds; source = Gleis-Label (leer = keine)
...
- `headlines`: **mehrere konfigurierte RSS-Feeds** (Default: **ZEIT** `https://newsfeed.zeit.de/all` + **NDR Hamburg** `https://www.ndr.de/nachrichten/hamburg/index-rss.xml`). Je Feed Top ~3 `{title, summary}` (`description` als summary, **HTML strippen**), dann **verschränkt** (ZEIT · NDR · ZEIT · NDR …); `source` = Feed-Label fürs Gleis-Label. Eigener `services/news.py` (oder den Podcast-Feed-Parser nachnutzen — **muss RSS 2.0 *und* RDF/RSS-1.0** können: NDR liefert RDF mit Items unter `rdf:RDF`, nicht unter `channel`; Redirects folgen). Config `kiosk_news.feeds = [{id,label,url}]`. 60 s–5 min cachen. **Nicht** mehr die zwei `input_text`-Entitäten.
...
3. **Tafel-Registry** (Schnittstelle oben): `weather`, `photo`, `film`, `series`, `nowplaying` — plus **eine Headline-Tafel pro Eintrag in `data.headlines[]`** (je Story eine Tafel; Gleis-Label = `headline.source`, z. B. „ZEIT" / „NDR · HAMBURG"). `render()` je Tafel nach Daten-Vertrag. Tafeln ohne Inhalt (`has()===false`) fallen raus.
